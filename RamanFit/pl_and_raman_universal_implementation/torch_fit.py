...
class Gaussian(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        return - (grad_output * input * torch.exp(-(input ** 2)/2))
...
class PLOnePeak(torch.nn.Module):
    def __init__(self, 
                 height, 
                 width,
                 x_coordinates_len,
                 ...
                 device,
                 dtype,
                 Gaussian_function=Gaussian.apply,
                 ):
...
def one_peak_map_fit(
    ...
    initial_fit_params, _ = single_peak_fit(np.mean(y_input, axis=(0,1))[spec_range], x_input[spec_range], peak_0_center=peak_0_center_initial_guess)
    init_params = {
        'peak_0_amplitude_guess':
    ...
    fitted_params_array = np.array([
        fitted_params['peak_0_centers'],
        fitted_params['peak_0_amplitudes'],
        fitted_params['peak_0_sigmas'],
        fitted_params['slopes'],
        fitted_params['intercepts']
    ])

    return fitted_params_array

def two_peak_map_fit(
                    x_input, 
                    y_input,
                    peak_0_center_initial_guess,
                    peak_1_center_initial_guess,
                    spec_cutoff_range,
                    learning_rate=0.1,
                    iterations=10000,
                    use_GPU=True,
    ...
    fitted_params_array = np.array([
        ...
        fitted_params['peak_0_amplitudes'],
        fitted_params['peak_0_sigmas'],
        fitted_params['peak_1_centers'],
        fitted_params['peak_1_amplitudes'],
        fitted_params['peak_1_sigmas'],
        fitted_params['slopes'],
        fitted_params['intercepts']
    ])
