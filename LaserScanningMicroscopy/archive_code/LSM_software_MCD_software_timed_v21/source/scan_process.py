...
class LSM_scan:
    ...
    def save_data(self):
        ...
        for channel_id in range(self.channel_num):
            ...
            with open(filepath + self.channel_names[channel_id] + '.png', "wb") as file:
                file.write(image_data)

            np.save(
                (filepath + f'data/trace_data_' + self.channel_names[
                    channel_id]), 
                       self.data[channel_id])
            
            np.save(
                (filepath + f'data/retrace_data_' + self.channel_names[
                    channel_id]), 
                       self.data[channel_id])
            
            np.savetxt(
                (filepath + f'data_text/trace_data_' + self.channel_names[
                    channel_id] + '.csv'), 
                       self.data[channel_id], 
                       delimiter=',')
            np.savetxt(
                (filepath + f'data_text/retrace_data_' + self.channel_names[
                    channel_id] + '.csv'), 
